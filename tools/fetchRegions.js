...
import { COUNTRIES, CONTINENT_NAMES } from './regionCodes.js';
...
export async function buildRegions({ force = false, build = new Date().toISOString() } = {}) {
  loadEnv();
  ...
  async function fetchOne(country) {
    ...
    if (!force && existsSync(file)) {
      ...
      return { country, count: cached.count, indices: reconstruct(cached.deltas) };
    }
    let attempt = 0;
    for (;;) {
      try {
        ...
        if (codes === null) {
          console.warn(`[regions] ${country.code} (${country.name}) → 404, skipping`);
          return null;
        }
        const indices = [];
        ...
        attempt++;
        if ((e.status === 429 || (e.status >= 500 && e.status < 600)) && attempt <= MAX_RETRIES) {
          const backoff = 500 * 2 ** attempt;
          console.warn(`[regions] ${country.code} ${e.status}, retry ${attempt} in ${backoff}ms`);
          await sleep(backoff);
          continue;
        }
  ...
  for (let i = 0; i < COUNTRIES.length; i += CONCURRENCY) {
    ...
    console.log(`[regions] ${Math.min(i + CONCURRENCY, COUNTRIES.length)}/${COUNTRIES.length}`);
  ...
  for (const r of results) {
    ...
    indexRegions.push({ code: country.code, type: 'country', en: country.name, fr: country.name, continent: country.continent, count });
    if (!continentSets.has(country.continent)) continentSets.set(country.continent, new Set());
  ...
  for (const [cont, set] of continentSets) {
    ...
    writeFileSync(join(REGIONS_DIR, `${code}.json`), JSON.stringify({ code, count: indices.length, deltas: deltaPack(indices) }));
  ...
  writeFileSync(join(REGIONS_DIR, '_index.json'), JSON.stringify({ build, regions: indexRegions }));
  const countries = indexRegions.filter((r) => r.type === 'country').length;
  console.log(`[regions] wrote ${countries} countries + ${continentSets.size} continents + world`);
